...
from .app_ui import WhatsappUI
from .app_state import WhatsappState

log = logging.getLogger()

class WhatsappApplication(Application):
    config: WhatsappConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: WhatsappUI = None
        self.state: WhatsappState = None

    async def setup(self):
        self.ui = WhatsappUI()
        self.state = WhatsappState()
